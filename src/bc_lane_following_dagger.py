...
def reset_env(env,town_name="Town07"):
    # nonlocal env
    if not env is None:
            env.close()
...
def run_eval(env,agent,replay_buffer=None):
            ...
            for _ in range(FLAGS.eval_episodes):
                ...
                if replay_buffer and not expert_agent:
                    expert_agent=reset_agent(env=env)
                if replay_buffer and expert_agent:
                            expert_agent.set_destination(env.unwrapped.core.destination.transform.location)
                eval_done = False
                episode_reward = 0
                ...
                # try:
                    ...
                    eval_action=np.nan_to_num(eval_action)
                    # except:
                    #     pass
                    if replay_buffer and agent:
                        control = expert_agent.run_step()
                        action = np.array([control.steer,control.throttle])
                        
                    next_eval_obs, eval_reward, eval_done, eval_truncated, eval_info = env.step(eval_action)
                    episode_reward += eval_reward
                    if replay_buffer:
                        if is_agent_at_junction(env):
                            
                            for _ in range(10):
                                replay_buffer.overwrite(
                                    dict(
                                        ...
                                        masks=1.0-eval_done,
                                        dones=eval_done,
                                        next_observations=next_eval_obs,
                                    )
                                )
                        else: 
...
def main(_):
    ...
    # towns=['Town04',"Town03",""]
    eval_town="Town01"
    training_town="Town06"
    env=reset_env(env,town_name=eval_town)
    action_dim = 2
    ...
    while i <  FLAGS.max_steps + 1:
        ...
        if i % FLAGS.eval_interval == 0:
            # env=reset_env(env,town_name=training_town)
            eval_info=run_eval(env,agent,random.choice(expert_replay_buffers))
            # eval_info=run_eval(env,agent,None)
            save_checkpoint(agent,policy_folder,i)
