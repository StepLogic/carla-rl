...
def collect_basic_agent_data(town="Town05",max_dist=600,difficulty="easy",min_junction=1):
    
    env = CarlaEvalEnv(use_rgb=True,town=town,start_server=True,max_dist=max_dist)
...
if __name__ == "__main__":
    ...
    for stage in difficulty:
        for i in range(number_trajectories_per_difficulty):
            collect_basic_agent_data(stage["town"],stage["max_dist"],stage["name"],stage["min_junction"])