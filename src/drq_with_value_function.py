...
def _update_value(
    ...
    def mse_loss_fn(value_params: Params) -> Tuple[jnp.ndarray, Dict[str, float]]:
        
        values = value.apply_fn(
            {"params": value_params}, batch["observations"]
        )
        value_loss = ((qs - values) ** 2).mean()
        return value_loss, {
            "value_loss": value_loss,
        }

    grads, info = jax.grad(mse_loss_fn, has_aux=True)(value.params)
    new_value = value.apply_gradients(grads=grads)
    new_target_value_params = soft_target_update(
        new_value.params, target_value_params, tau
    )
    return new_target_value_params,new_value, info
...
class DrQLearner(Agent):
    def __init__(
        ...
        value_def = StateValueEnsemble(hidden_dims, num_qs=min(num_qs,2))
        value_def = PixelMultiplexer(
            encoder=encoder_def, network=value_def, latent_dim=latent_dim,stop_gradient=True
        )
        ...
        target_value_params = copy.deepcopy(value_params)
        ...
        self._value=value
        self._target_value_params=target_value_params
        self._temp = temp
        self._rng = rng
    ...
    def update(self, batch: FrozenDict,utd_ratio: int=None, enable_update_temperature: bool = True, output_range: Optional[Tuple[jnp.ndarray, jnp.ndarray]] = None) -> Dict[str, float]:
        ...
        (self._target_value_params,self._value,value_info)=_update_value(
            self._rng,self._actor,self._critic,self._value,self._target_value_params,batch,self.tau
        )
