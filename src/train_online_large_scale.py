...
def main(_):
    ...
    for path in glob.glob("datasets/*.pkl"):
        with open(path, 'rb') as f:
            ...
            expert_replay_buffer_iterator = expert_buffer.get_iterator(
                sample_args={"batch_size": FLAGS.batch_size})
            expert_replay_buffers.append(expert_replay_buffer_iterator)
    ...
    for i in tqdm.tqdm(
        ...
        if i >= FLAGS.start_training:
            ...
            for ix,expert_replay_buffer_iterator in enumerate(expert_replay_buffers):
                batch_expert = next(expert_replay_buffer_iterator)
                update_info_expert = agent.update(
                    batch_expert,
                    enable_update_temperature=False)
                if i % FLAGS.log_interval == 0:
                    logger.log_training(update_info_expert, i,prefix=f"_expert_{ix}")
                    logger.print_status(i, FLAGS.max_steps)
        # Periodic evaluation
