...
import gym.wrappers
from jaxrl5.data.hindsight_replay_buffer import HindsightReplayBuffer
...
# from vision_rl.stb3.jax_s
from vision_rl.rllib_integration.carla_env_v2 import CarlaEnv
from vision_rl.stb3.jax_sparse_goal_experiment import JAXSparseGoalExperiment
import numpy as np
experiment_config = {
    "framework": "torch",
    "num_workers": 1,
...
def relabel(batch: Dict[str, Any],resampled_batch:Dict[str, Any]) -> Dict[str, Any]:
    """Relabel experiences with info metrics."""
    batch_size = resampled_batch['actions'].shape[0]
...
def get_config():
    ...
    config.model.pixel_keys = ("obs","goal")
...
def train_agent(config):
    ...
    replay_buffer = HindsightReplayBuffer(
        ...
        'obs': (3,),     # 3x1 array values
        'goal': (3,),      # 3x1 array values
        "is_success":(1,),
        "distance_to_goal":(1,),
    ...
    for i in tqdm.tqdm(range(1, config.training.max_steps + 1),
        ...
        # while not done or not terminate:
            ...
            if done or terminate:
                # breakpoint()
                for k, v in info.pop('episode',{}).items():
