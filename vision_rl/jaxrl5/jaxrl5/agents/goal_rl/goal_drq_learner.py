...
from jaxrl5.agents.goal_rl.goal_rl_learner import GoalRLLearner
import optax
...
def _share_encoder(source, target):
    ...
    new_params = frozen_dict.unfreeze(frozen_dict.FrozenDict(target.params).copy(add_or_replace=replacers))
    return target.replace(params=new_params)


class DrQLearner(GoalRLLearner):
    ...
    def create(
        ...
        actor_cls = partial(TanhNormal,state_dependent_std=state_dependent_std, base_cls=actor_base_cls, action_dim=action_dim)
        # print("Encoder Frozen",freeze_encoder)
    ...
    def update(self, batch: DatasetDict, utd_ratio: int, update_temperature: bool = True,
               output_range: Optional[Tuple[jnp.ndarray, jnp.ndarray]] = None):
        new_agent = self

        # if "obs" not in batch["next_observations"]:
        #     batch = _unpack(batch)
        ...
        next_observations = self.data_augmentation_fn(key, batch["desired_goals"])
        batch = frozen_dict.unfreeze(frozen_dict.FrozenDict(batch).copy(
            add_or_replace={
                "observations": observations,
                "next_observations": next_observations,
                "desired_goals": next_observations,
            }
        ))

        new_agent = new_agent.replace(rng=rng)

        return GoalRLLearner.update(new_agent, frozen_dict.unfreeze(batch), utd_ratio,
                                 update_temperature=update_temperature, output_range=output_range)
