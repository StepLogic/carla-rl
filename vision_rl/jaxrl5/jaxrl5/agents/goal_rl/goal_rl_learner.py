...
class GoalRLLearner(SACLearner):
    critic: TrainState
    ...
    def update_actor(self, batch: DatasetDict, output_range: Optional[Tuple[jnp.ndarray, jnp.ndarray]]) -> Tuple[
        Agent, Dict[str, float]]:
        key, rng = jax.random.split(self.rng)
        key2, rng = jax.random.split(rng)

        def actor_loss_fn(actor_params) -> Tuple[jnp.ndarray, Dict[str, float]]:
            dist = self.actor.apply_fn({"params": actor_params}, batch["observations"],batch["desired_goals"], output_range=output_range)
        ...
        if not isinstance(grads, dict):
            grads = frozen_dict.unfreeze(grads)
        # breakpoint()
        assert grads.keys() == self.actor.params.keys(), "Gradients and parameters keys do not match!"
    ...
    def update_critic(self, batch: DatasetDict) -> Tuple[TrainState, Dict[str, float]]:
        ...
        dist = self.actor.apply_fn(
            {"params": self.actor.params}, batch["next_observations"],batch["desired_goals"]
        )
        ...
        def critic_loss_fn(critic_params) -> Tuple[jnp.ndarray, Dict[str, float]]:
            ...
            return critic_loss, {"critic_loss": critic_loss, "q": qs.mean(),"target q":target_q.mean()}
    ...
    def update(self, batch: DatasetDict, utd_ratio: int, update_temperature: bool = True,
        ...
        # breakpoint()
        new_agent, actor_info = new_agent.update_actor(mini_batch, output_range=jax.tree_util.tree_map(
