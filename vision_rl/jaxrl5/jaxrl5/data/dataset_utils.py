...
class ReconImageDataset(ImageDataset):
    ...
    def __init__(self,
        ...
        super().__init__(None,
                         ...
                         dones_float=None)
class ReconImageDatasetWithoutStates(MemoryEfficientReplayBuffer):
    ...
    def __init__(self,
        ...
        self.observations_space=gym.spaces.Dict(
            {
                "obs": gym.spaces.Box(
                    low=0,
                    high=255,
                    shape=trajectories["image"][0]["obs"].shape,
                   dtype=np.uint8,
                ),
                "goal":  gym.spaces.Box(
                    low=0,
        ...
        # print("shape",trajectories["image"][0]["obs"].shape)
        # breakpoint()
        self.action_space= gym.spaces.Box(low=-1.0, high=2.0, shape=np.shape(trajectories["actions"][0]), dtype=np.float32)
    
        super().__init__(
        self.observations_space,
        self.action_space,
        len(trajectories["image"]), pixel_keys=("obs","goal")
        )
 
    

        for i in  range(len(trajectories["image"])):
            if trajectories['dones'][i] == 1:
                reward=0
            else:
            ...
            if i ==len(trajectories["image"])-2:
                done=1
            try:
                self.insert(
                    dict(observations=trajectories["image"][i],
                        actions=trajectories["actions"][i],
                        rewards=reward,
                        masks=1-trajectories['dones'][i],
                        dones=done,
                        next_observations=trajectories["next_image"][i]))
            except:
                continue
        # dataset['observations'] = trajectories["image"]
        # dataset['next_observations'] = trajectories["next_image"]
        # dataset['actions'] = transform_action(trajectories["actions"])
        # dataset['terminals'] = trajectories["dones"]
        # dataset['rewards'] = []
