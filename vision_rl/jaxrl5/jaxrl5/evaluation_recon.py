...
import jaxrl5.utils.np_utils as np_utils
...
def evaluate(agent, batch, dir, istraj=False) -> Dict[str, float]:
    stats = {'norm diff': []}
    norms = []

    for obs,goal,actual_waypoints in zip(batch["observations"]["obs"],batch["observations"]["goal"],batch["actions"]):
        # obs = batch["observations"]
        # breakpoint()
        obs= FrozenDict(dict(obs=obs,goal=goal))
        # img = batch.image_observations[i]

        pred_action = agent.eval_actions(obs)
        # actual_waypoints = batch["actions"]
