...
class HerReplayBufferModified(HerReplayBuffer):
    ...
    def _get_virtual_samples(
        ...
        new_goals,image_goals = self._sample_goals(batch_indices, env_indices)
        obs["desired_goal"] = new_goals
        # The desired goal for the next observation must be the same as the previous one
        next_obs["desired_goal"] = new_goals
        obs["goal"]=image_goals
        next_obs["goal"]=image_goals
        assert (
    ...
    def _sample_goals(self, batch_indices: np.ndarray, env_indices: np.ndarray) -> np.ndarray:
        ...
        return self.next_observations["achieved_goal"][transition_indices, env_indices],self.next_observations["observation"][transition_indices, env_indices]
