...
from stbl3_experiments import STBL3Experiment
...
def main():
    parser = argparse.ArgumentParser(description="DQN training script for CARLA")
    ...
    #     config["env"] = CarlaEnv
    #     config["env_config"]["experiment"]["type"] = STBL3Experiment
    #     # config["callbacks"] = SACCallbacks

    # print(config.keys())
    # Create environment
    # env = DummyVecEnv([lambda: Monitor(CarlaEnv(config["env_config"]))])
    ...
    # Setup callbacks
    callbacks = [
        CheckpointCallback(save_freq=10000, save_path=args.output_dir),
    ]
